...
public class IotDBKafkaMetricStoreTest {
    ...
    public void testUsesPreparedBatchesForMetricRows() throws Exception {
        ...
        Mockito.when(statement.executeBatch()).thenReturn(new int[] {Statement.SUCCESS_NO_INFO});
    ...
    @Test
    public void testDriverReportedFailureIsNotAcknowledged() throws Exception {
        DataSource dataSource = Mockito.mock(DataSource.class);
        Connection connection = Mockito.mock(Connection.class);
        PreparedStatement statement = Mockito.mock(PreparedStatement.class);
        Mockito.when(dataSource.getConnection()).thenReturn(connection);
        Mockito.when(connection.prepareStatement(Mockito.anyString())).thenReturn(statement);
        Mockito.when(statement.executeBatch()).thenReturn(new int[] {Statement.EXECUTE_FAILED});
        KafkaMetricWriteBatch batch = new KafkaMetricWriteBatch("7", "9", "kafka", Instant.now(), 0L,
            KafkaMetricCollection.builder().build());
        Assertions.assertThrows(KafkaMetricStorageException.class,
            () -> new IotDBKafkaMetricStore(dataSource).write(batch));
