 ...
 */
final class KafkaMetricQueryService {
    ...
    KafkaMetricQueryService(DataSource dataSource) {
    ...
    void validate(SingleGeneralReportDO report) {
        Objects.requireNonNull(report, "report");
        buildQuery(report, resolve(report));
    }
    ...
    private QueryStatement buildQuery(SingleGeneralReportDO report, MetricQuery metric) {
        ...
        if (report.getStartTime() != null && report.getEndTime() != null
            && !report.getStartTime().isBefore(report.getEndTime())) {
    ...
    private static void bind(PreparedStatement statement, List<Object> parameters) throws SQLException {
        for (int index = 0; index < parameters.size(); index++) {
            ...
            if (parameter instanceof Timestamp timestamp) {
                ...
                KafkaJdbcParameters.setString(statement, index + 1, parameter.toString());
            }
        }
    }
    ...
    private static final class MetricQuery {

        KafkaIotDBSchema.TableDefinition table;

        String field;

        private MetricQuery(KafkaIotDBSchema.TableDefinition table, String field) {
    ...
    @Value
    @Accessors(fluent = true)
    private static final class QueryStatement {

        String sql;

        List<Object> parameters;

        private QueryStatement(String sql, List<Object> parameters) {
