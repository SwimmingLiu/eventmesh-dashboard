 ...
 */
public final class KafkaMetricCollect extends AbstractCollect implements AutoCloseable {
    ...
    private final DataSyncHandler synchronization = new DataSyncHandler();
    ...
    KafkaMetricCollect(KafkaCollectConfig config, KafkaMetricStore store, Clock clock, SDKManage sdkManage) {
        ...
        clusterMetadata.setId(config.getClusterId());
        clusterMetadata.setOrganizationId(config.getOrganizationId());
        clusterMetadata.setName(config.getClusterName());
        clusterMetadata.setClusterType(ClusterType.STORAGE_KAFKA_CLUSTER);
        setClusterMetadata(clusterMetadata);
        CollectMetadata collectMetadata = new CollectMetadata();
        collectMetadata.setClusterId(config.getClusterId());
        collectMetadata.setClusterType(ClusterType.STORAGE_KAFKA_CLUSTER);
        setCollectMetadata(collectMetadata);
        synchronization.setBatchWriter(batches -> batches.values().forEach(samples -> samples.forEach(sample ->
    ...
    public void request() {
        ...
        if (closed.get() || now.isBefore(nextCollectionAt) || !collecting.compareAndSet(false, true)) {
            return;
        }
        try {
            synchronized (this) {
                if (!closed.get()) {
                    collect(0, synchronization.getDataSyncHandlerWrapper(1));
                }
            }
        } finally {
    ...
    protected void doCollect() {
        setData(new KafkaMetricSample(coordinator.collect(organizationId, clusterId, clusterName, sdkClient, endpoints)));
    }
