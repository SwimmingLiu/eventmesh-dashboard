...
/** Produces all Consumer Group-dimension samples from the AdminClient source snapshot. */
final class KafkaGroupMetricCollector implements KafkaDimensionMetricCollector {
    ...
    public void collect(KafkaMetricCollectionContext context, KafkaMetricCollection.Builder result) {
        context.adminMetrics().groups().forEach(sample -> result.add(sample.metric(), sample.group(),
            sample.topicPartition().orElse(null), sample.value(), sample.collectedAt()));
    }
}
